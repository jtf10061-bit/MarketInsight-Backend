...
def api_document_qa(req: DocumentQARequest):
    ...
    response = ai_client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "あなたはドキュメントQAアシスタントです。提供されたコンテキストに基づいて回答してください。コンテキストに直接書かれていない場合は、コンテキストの内容から推論して回答し、その旨を明記してください。コンテキストと全く関係ない質問には「該当する情報が見つかりません」と答えてください。回答はMarkdown形式で、見出し・箇条書き・太字を使って読みやすく整形してください。",
            },
